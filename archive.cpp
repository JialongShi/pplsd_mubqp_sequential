...
bool archive::loadFromFile(const char* filename) {
	...
	if (archiveSize > 0) {
		...
		for (int line = 0; line < archiveSize; line++) {
			for (int mi = 0; mi < m; mi++) {
				if (fscanf(iniAFile, "%lf", &iniSol.fitness[mi]) == 0) {
...
bool archive::printToFile(const char* filename) {
	...
	if (!solList.empty()) {
		...
		for (list<solution>::iterator itSol = solList.begin(); itSol != solList.end(); itSol++) {
			for (int mi = 0; mi < m; mi++) {
				fprintf(resultSolFile, "%.0f ", itSol->fitness[mi]);
			}
			for (int i = 0; i < n; ++i) {
