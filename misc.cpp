...
double moveCost1BitFlip(const int &mi, const int &k, const solution &sol, const problem &inst) {
	int n = inst.n;

	double deltaFitness = 0;

	if (sol.x[k] == 0) {
		deltaFitness += inst.Q[mi][k][k];
		for (int j = 0; j < n; j++) {
			if (j == k) continue;
			deltaFitness += sol.x[j] * inst.Q[mi][j][k];
			deltaFitness += sol.x[j] * inst.Q[mi][k][j];
		}
	}
	else { //if (sol.x[k] == 1)
		deltaFitness -= inst.Q[mi][k][k];
...
bool move1BitFlip(const int &k, solution &sol) {
	sol.x[k] = 1 - sol.x[k];
	return true;
}
...
solution genNeighborSol(const problem &inst, const solution &sol, const int k) {
	solution solPrime = sol;
	for (int mi = 0; mi < inst.m; mi++) {
		solPrime.fitness[mi] += moveCost1BitFlip(mi, k, sol, inst);
	}
	move1BitFlip(k, solPrime);
	return solPrime;
}

solution genNeighborSol_WithoutFitness(const problem &inst, const solution &sol, const int k) {
	solution solPrime = sol;
	move1BitFlip(k, solPrime);
	return solPrime;
}
